...
def velboxplot(limits, clambda, data= [], xdata = [], paperdata = [],
    ...
    if datatype == 'X-Shoot':
        ...
        if len(xrwave) > 0:
            ...
            res = optmin(boxminfunc, [-100.0, 200.0, 0.0, 0.5], args = (xrvel, csflux), method = 'Nelder-Mead', tol = 0.001)
            #resx = getparams(res.params)
            resx = res.x
            boxsigma = np.std(csflux[np.where((xrvel <= resx[0]) | (xrvel >= resx[1]))])
            xbox = boxsigma
    ...
    if datatype == 'Keck':
        if  '4923' in title:
            ...
            data = binarray(kecktemp[0], kecktemp[1], kecktemp[2], len(kecktemp[0])/10.0)
        if len(data) > 0:
            ...
            if len(krwave) > 0:
                ...
                res = optmin(boxminfunc, [-100.0, 200.0, 0.0, 0.5], args = (krvel, csflux), method = 'Nelder-Mead', tol = 0.001)
                #resx = getparams(res.params)
                resx = res.x
                boxsigma = np.std(csflux[np.where((krvel <= resx[0]) | (krvel >= resx[1]))])
                if xshootsig >= 3.0:
                    fluxlist = np.append(fluxlist, csflux)
    ...
    if datatype == 'Xu-HIRES':
        ...
        if len(paperhires) > 0:
            ...
            if ((len(prwave) > 0) & (title != 'CaII 8498')):
                ...
                res = optmin(boxminfunc, [-100.0, 200.0, 0.0, 0.5], args = (prvel, csflux), method = 'Nelder-Mead', tol = 0.001)
                #resx = getparams(res.params)
                resx = res.x
                boxsigma = np.std(csflux[np.where((prvel <= resx[0]) | (prvel >= resx[1]))])
                if xshootsig >= 3.0:
                    #print clambda, resx[-1]/(1.0*boxsigma)
                    ax.plot(prvel, csflux, '--', color = 'orange', label = 'Xu HIRES')
                    datavel = prvel
                    fluxlist = np.append(fluxlist, csflux)
...
if __name__ == '__main__':
    ...
    #clambdas = [4351.735, 4549.465, 4233.162, 4583.829, 4923.921, 5316.609,
    #    3933.653, 3968.476]
    #titles = ['Fe I 4351-735', 'Fe I 4549-465', 'Fe II 4233-162',
    #    'Fe II 4583-829', 'Fe II 4923-921', 'Fe II 5316-609', 'Ca II 3933-653',
    #    'Ca II 3968-476']
    keck = keckread()
    xshoot = xshootread()
    model = conmodelread()
    ...
    mask = np.where((clambdas > 3422.0) & (clambdas < 6200.0))[0]
    ...
    for i in range(0,len(clambdas)):
        res, datax, csy = velboxplot(limitarr[i], clambda = clambdas[i], data = keck, model = model,
            title = titles[i], mode = 'SAVE', xdata = xshoot, paperdata = hires,
            datatype = 'Keck', dataoff = 0.0, writename = 'XSFe_II_testlines.txt')

    fname = 'CaIIlines.txt'
    temptitles = np.loadtxt(fname, usecols = [3,4], dtype = 'string')
    clambdas = 10.0*np.loadtxt(fname, usecols = [0])
    mask = np.where((clambdas > 3250.0) & (clambdas < 6200.0))[0]
    clambdas = clambdas[mask]
    temptitles = temptitles[mask]
    titles = [temptitles[i,0] + ' ' + temptitles[i,1] + ' ' + str(clambdas[i]).replace('.', '-') for i in range(0,len(clambdas))]
    limitarr = [(vel2wave(-700.0, clambda), vel2wave(+700.0, clambda)) for clambda in clambdas]
    for i in range(0,len(clambdas)):
        res, datax, csy = velboxplot(limitarr[i], clambda = clambdas[i], data = keck, model = model,
            title = titles[i], mode = 'SAVE', xdata = xshoot, paperdata = hires,
            datatype = 'Keck', dataoff = 0.0, writename = 'XSCa_II_testlines.txt')
    '''
    ...
    #temptitles = temptitles[mask]
    clambdas = [4923.921]
    titles = ['Fe' + ' ' + 'II' + ' ' + str(clambdas[i]).replace('.', '-') for i in range(0,len(clambdas))]
    limitarr = [(vel2wave(-700.0, clambda), vel2wave(+700.0, clambda)) for clambda in clambdas]
    for i in range(0,len(clambdas)):
        res, datax, csy = velboxplot(limitarr[i], clambda = clambdas[i], data = keck, model = model,
            title = titles[i], mode = 'SAVE', xdata = xshoot, paperdata = hires,
            datatype = 'Keck', dataoff = 0.0, writename = 'XSCFe_I_testlines.txt')
